<section id="what">
	<h1>WTF Is This?</h1>
	<p>
		<?php echo \Html::anchor('/', 'WTF Is My PageRank'); ?> is a simple
		tool offering webmasters the ability to check their PageRank.
	</p>
</section>
<section id="pagerank">
	<h1>WTF Is A PageRank?</h1>
	<p>
		PageRank (AKA PR) is the term Google created in reference to their
		algorithm which ranks web pages. The term PageRank is derived from the
		co-founder of Google, "Larry Page", who along with Sergey Brin developed
		this algorithm.
	</p>
	<p>
		PageRank is a scale that Google uses internally to value the importance
		of each and every web page. The <em>exact</em> PageRank of web pages
		is not published, Google does however report whole value numbers based
		on a 0-10 scale. It's worth noting that the real PageRank may have a
	...
	<p>
		PageRank is a voting structure that calculates the important of web
		pages by the number of incoming links to them. Normally, the more
		incoming links a page has, the higher it's PageRank value will be;
		however, the PageRank algorithm is rather complex and will take into
	...
	<p>
		...
		assigned a PageRank of 3. This is of course not a hard and fast rule,
		as the internal link structure of a site will have an impact on how
		the PageRank 'flows' through the pages of a site. At times, many
		internal pages may have a value equal to that of the main page or have
		no PageRank at all. To get the highest possible PageRank for all the
	...
	<p>
		...
		we obtain your PageRank) does not always display your latest PageRank
		...
		Google PageRank for a site or page, but the most accurate number will
	...
	<p>
		...
		website. It is a difficult task to achieve a PageRank higher than 6
	...
	<p>
		Sites with a higher PageRank will normally have a better Google ranking
		for keywords appearing on their websites; however, PageRank is not the
	...
	<p>
		For further reading on PageRank algorithm see
